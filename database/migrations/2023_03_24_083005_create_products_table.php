    ...
    public function up()
    {
        Schema::create('products', function (Blueprint $table) {
           ...
           // $table->decimal('region_transfer', 10, 2)->default(0)->comment('item that is transfer from region, it will be added to quatity fro transfer again to other regions');
            ...
            $table->decimal('main_store_qty', 10, 2)->default(0)->comment('total product recieved that will be use for distribution and for repair');
            $table->decimal('main_store_distributed_qty', 10, 2)->default(0)->comment('product distribute to the employee');
            $table->decimal('main_store_sold_qty', 10, 2)->default(0)->comment('products sold from main store');
            $table->decimal('damage_quantity', 10, 2)->default(0)->comment('how many product product has been dismantlenot necessary required');
            $table->decimal('region_store_qty', 10, 2)->default(0)->comment('total product recieved that will be use for distribution and for repair');
            $table->decimal('region_store_distributed_qty', 10, 2)->default(0)->comment('item that is transfer from region, it will be added to quatity fro transfer again to other regions');
            $table->decimal('region_store_sold_qty', 10, 2)->default(0)->comment('products sold from region store');
            $table->decimal('extension_store_qty', 10, 2)->default(0)->comment('total product recieved that will be use for distribution and for repair');
            $table->decimal('extension_store_sold_qty', 10, 2)->default(0)->comment('products sold from extension store');
            $table->decimal('extension_distributed_qty', 10, 2)->default(0)->comment('products distributed from extension store');
